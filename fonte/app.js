...
const CamadaDGT = L.TileLayer.extend({
  getTileUrl: function (c) { return dgtUrl(this._getZoomForUrl(), c.x, c.y, PX_DGT); },
});
  ...
  navigator.serviceWorker.addEventListener('message', (e) => {
    ...
    if (d.tipo === 'quantos') {
      ...
      if (b && !aDescarregar && !/A guardar|A preparar|✓/.test(b.textContent)) {
        ...
        const mb = satModo === 1 ? Math.round(d.faltam * 0.73) : 0;
...
function actualizaContagem() {
  ...
  const mb = satModo === 1 ? Math.round(u.length * 0.73) : 0;
