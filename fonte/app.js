...
function ligaArvores(map, op) {
  ...
  const TECTO = op.tecto == null ? 14000 : op.tecto;
  ...
  // Numero proprio de cada arvore, tirado so da sua posicao: nao muda com o
  // mapa, por isso o sorteio do tecto da sempre o mesmo conjunto.
  //
  // A primeira versao usava o fract(sin(...)) de manual de shaders. Nao serve
  // AQUI: esse truque precisa que as coordenadas variem entre 0 e 1, e as
  // coordenadas mercator variam 1e-5 ao longo de uma vista inteira. Nesse
  // intervalo o seno e praticamente uma recta, e o fract de uma recta da
  // FAIXAS -- arvores as riscas com corredores vazios pelo meio, que foi o que
  // ele fotografou. Agora e um hash de inteiros, que da ruido a serio.
  function sorte(mx, my) {
    const a = (mx * 134217728) | 0, b = (my * 134217728) | 0;   // ~0,3 m por unidade
    return baralha(a, b, 11);
  }
  ...
  function semeia(soReusar) {
    ...
    let faltouDEM = false, faltouTempo = false;
    {
      for (const [d, tx, ty] of lista) {
        const salto = d < DIST[0] ? 1 : (d < DIST[1] ? 3 : 9);
        ...
        if (c.length) { partes.push(c); total += c.length / 11; }
      }
    }
    semTerreno = faltouDEM;
    incompleto = faltouTempo;

    // O tecto NAO se aplica cortando na ordem em que as celulas saem. Foi
    // assim a primeira vez e deu um erro com forma: as celulas percorrem-se
    // de noroeste para sudeste, as do principio gastavam o tecto todo e as do
    // fim ficavam sem uma unica arvore -- meio mapa com floresta e meio sem,
    // com uma fronteira recta a separar. Corta-se por um numero tirado da
    // POSICAO da arvore, que e sempre o mesmo: o conjunto que passa e o mesmo
    // esteja o mapa onde estiver, e esta espalhado por igual.
    const manterG = total > TECTO ? TECTO / total : 1;
    ...
    for (const c of partes) {
      ...
      for (let t = 0; t < nc; t++) {
        if (k >= TECTO) break;
        const o = t * 11;
        if (manterG < 1 && sorte(c[o], c[o + 1]) > manterG) continue;
        buf.set(c.subarray(o, o + 11), k * 11); k++;
      }
      if (k >= TECTO) break;
    }
    INST = buf; nInst = k;
