...
const map = L.map('map', { zoomControl: false, attributionControl: false, maxBounds: [[39.55, -8.85], [41.15, -6.45]], maxBoundsViscosity: 0.8, minZoom: 9, maxZoom: 20, preferCanvas: true, tap: false });
...
// Os caminhos do OSM sao dezenas de milhares de linhas e apareciam sozinhos a
// partir de certo zoom. Passam a depender de um botao, e comeca desligado: o
// mapa abre limpo e ele liga quando quer ver por onde se pode ir.
let camLigados = false;
try { camLigados = localStorage.getItem('sne-caminhos') === '1'; } catch (e) {}
function apagaCaminhos() {
  OSM_KINDS.forEach((k) => { const g = osmGroups[k]; g.remove(); osmClear(k, g); });
  const nota = document.getElementById('cam-note'); if (nota) nota.hidden = true;
}
function osmByZoom() {
  if (!camLigados) { apagaCaminhos(); return; }
...
function focusOn(r) {
  ...
  loadOSM();
  // Escolher um percurso e mandar mostra-lo: fica na lista dos visiveis para
  // continuar la quando ele largar a seleccao.
  VISIVEIS.add(r.id);
  layers.forEach((g, id) => { if (id !== r.id) g.remove(); });
  const gr = camadaDe(r);
  gr.addTo(map);
  if (r.lidar) gr.eachLayer((l) => l.bringToFront && l.bringToFront());
  pintaVistos();
  osmByZoom();
...
function unfocus() {
  ...
  VISIVEIS.forEach((id) => { const g = layers.get(id); if (g) g.addTo(map); });
  soAVista();
  osmByZoom();
...
// O botao que era do satelite passa a ligar e desligar os caminhos. Nao ha
// satelite nenhum para ligar, e o lugar ja estava feito.
function pintaBotaoCaminhos() {
  const b = document.getElementById('btn-sat'); if (!b) return;
  b.setAttribute('aria-pressed', camLigados ? 'true' : 'false');
  b.setAttribute('aria-label', camLigados ? 'Esconder os caminhos' : 'Mostrar os caminhos');
  b.title = camLigados ? 'Caminhos: ligados' : 'Caminhos: desligados';
}
{
  const b = document.getElementById('btn-sat');
  if (b) {
    b.onclick = () => {
      camLigados = !camLigados;
      try { localStorage.setItem('sne-caminhos', camLigados ? '1' : '0'); } catch (e) {}
      pintaBotaoCaminhos();
      if (camLigados) loadOSM();
      osmByZoom();
      toast(camLigados ? 'Caminhos ligados.' : 'Caminhos desligados.');
    };
    pintaBotaoCaminhos();
  }
}
...
// ---- Ocupacao do solo: o codigo verdadeiro da COS -------------------------
...
const PADROES = {"floresta":"data:image/png;base64,","montado":"data:image/png;base64,","matos":"data:image/png;base64,","rocha":"data:image/png;base64,"};
...
// Todos os tracados, em [lon, lat], tirados do que ja esta em memoria. Nao
// dependem de nenhum azulejo ter chegado, por isso o corredor sem vegetacao
// ao longo do caminho e o mesmo a primeira vez e a decima.
let TRACADOS = null;
function tracadosTodos() {
  if (TRACADOS) return TRACADOS;
  TRACADOS = [];
  for (const r of ((D && D.routes) || [])) {
    let ls = geo(r);
    if (!ls.length && r.segs) ls = r.segs.map(segPts);
    for (const l of ls) if (l && l.length > 1) TRACADOS.push(l.map((q) => [q[1], q[0]]));
  }
  return TRACADOS;
}
  ...
  const por = () => {
    ...
    try {
      ARV = ligaArvores(mapT, {
        ...
        // O corredor limpo ao longo dos percursos tem de ser sempre o mesmo,
        // esteja o azulejo das rotas carregado ou nao. A lista de percursos ja
        // esta toda em memoria -- e ela que manda.
        rotas: tracadosTodos,
        aoContar: (n) => {
          ...
          if (n > 0 && !avisoArv) {
            avisoArv = true;
            toast('Árvores, matos e rocha onde a carta do solo os marca. Alturas modeladas, não medidas.');
          }
        },
      });
...
// Sem o ficheiro guardado, CADA pedaco do mapa topografico e um pedido a rede.
// Medido: abrir sao 9 pedidos e 2,5 MB; cada arrasto sao 3 a 9 pedidos. Num
// telemovel isso e meio segundo de espera por gesto, antes de desenhar seja o
// que for -- e o que ele sentia como "muito lento". O mapa topografico e agora
// o unico mapa, por isso isto deixa de estar escondido num painel.
let avisouRede = false;
function avisaTopoRede() {
  if (avisouRede || !topoAberto || (offEstado && offEstado.topo)) return;
  avisouRede = true;
  toastAction('Este mapa está a vir da rede aos pedaços — cada arrasto são várias idas à internet. Guardado no telemóvel fica instantâneo, e funciona na serra sem sinal.',
    'Guardar (91 MB)',
    () => { toast('A guardar o mapa topográfico…'); mandaSW({ tipo: 'guardar-topo' }); });
  setTimeout(() => { const el = document.getElementById('toast');
    if (el && el.querySelector('#t-act')) el.hidden = true; }, 15000);
}
...
// ---- os percursos nascem quando ele os manda mostrar ----------------------
// Antes, os 133 percursos eram todos construidos e postos no mapa ao abrir:
// 22 mil formas e meio segundo de bloqueio, para depois ele olhar para um.
// Agora a camada de um percurso so se constroi da PRIMEIRA vez que ele o pede,
// e o mapa abre sem nenhum -- e a ideia dele, e e melhor do que a minha.
function constroiRota(r) {
  const g = L.layerGroup();
  if (r.lidar && r.segs) {
    // O shadeCol so devolve TRES cores, e o contorno e todo da mesma: os
    // trocos juntam-se por cor numa polilinha de varias partes. Quatro formas
    // por percurso em vez de cento e setenta, com o mesmo desenho.
  ...
  if (c) caixaRota.set(r.id, c);
  return g;
}
const caixaRota = new Map();
function camadaDe(r) {
  let g = layers.get(r.id);
  if (!g) { g = constroiRota(r); layers.set(r.id, g); }
  return g;
}

// VISIVEIS sao os que ele mandou mostrar. Comeca vazio de proposito.
const VISIVEIS = new Set();
function mostraRota(r, sim) {
  const g = camadaDe(r);
  if (sim) {
    VISIVEIS.add(r.id);
    if (!selected || selected.id === r.id) {
      g.addTo(map);
      if (r.lidar) g.eachLayer((l) => l.bringToFront && l.bringToFront());
    }
  } else {
    VISIVEIS.delete(r.id);
    g.remove();
  }
}
function todasAsRotas(sim) {
  D.routes.forEach((r) => mostraRota(r, sim));
  pintaVistos();
  toast(sim ? 'Todos os percursos no mapa.' : 'Percursos escondidos.');
}
function pintaVistos() {
  document.querySelectorAll('.olho').forEach((b) => {
    const on = VISIVEIS.has(+b.dataset.olho);
    b.setAttribute('aria-pressed', on ? 'true' : 'false');
    b.textContent = on ? '◉' : '○';
    b.title = on ? 'Tirar do mapa' : 'Mostrar no mapa';
  });
  const t = document.getElementById('mostrar-todos');
  if (t) t.textContent = VISIVEIS.size >= D.routes.length ? 'Esconder todos' : 'Mostrar todos';
}

// So se desenha o que esta a vista. Com poucos percursos mostrados isto quase
// nao tem trabalho; com "mostrar todos" ligado volta a valer a pena.
function soAVista() {
  if (selected) return;
  const vb = map.getBounds().pad(0.3);
  layers.forEach((g, id) => {
...
function sheetIdle() {
  ...
  $('sheet').innerHTML = `<div class="grab"></div>
    <p class="hint">O mapa abre limpo, de propósito. Na <strong>Lista</strong> tocas no <strong>círculo</strong> de cada percurso para o pôr no mapa — ou em <strong>Mostrar todos</strong>, se quiseres ver tudo. O botão <strong>⌾</strong> à direita liga os caminhos do OSM. Em <strong>Criar</strong> montas o teu próprio trilho.</p>
    <div class="stats">
    ...
    <p class="note">Nada disto é desenhado sem tu mandares: são 133 percursos e 20 mil quilómetros de caminhos, e desenhá-los todos ao abrir era o que tornava a aplicação pesada. A cor dos percursos marcados mostra a sombra medida por LiDAR, onde existe.</p>
    ${blocoOffline()}`;
...
function renderList() {
  ...
  $('items').innerHTML = rs.map((r) => {
    ...
    return `<button class="item" data-id="${r.id}">${prBadge(r)}<span><span class="nm">${esc(r.ref ? r.ref + ' · ' : '')}${nome}</span><br><span class="sub">${sub}</span></span>${v}<span class="olho" data-olho="${r.id}" role="button" tabindex="0" aria-pressed="false">\u25CB</span></button>`;
  }).join('');
  bindItems($('items'));
  pintaVistos();
}
function bindItems(root) {
  root.querySelectorAll('.item').forEach((b) => b.onclick = () => select(D.routes.find((r) => r.id === +b.dataset.id), true));
  // o olho mostra e esconde sem escolher: tocar nele nao deve abrir a ficha
  root.querySelectorAll('.olho').forEach((o) => {
    const faz = (e) => {
      e.stopPropagation(); e.preventDefault();
      const r = D.routes.find((x) => x.id === +o.dataset.olho);
      if (r) { mostraRota(r, !VISIVEIS.has(r.id)); pintaVistos(); }
    };
    o.onclick = faz;
    o.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') faz(e); };
  });
}
// "Mostrar todos" ao lado da contagem, feito aqui para nao mexer no html
{
  const c = document.getElementById('count');
  if (c && !document.getElementById('mostrar-todos')) {
    const b = document.createElement('button');
    b.id = 'mostrar-todos'; b.type = 'button'; b.className = 'chip';
    b.textContent = 'Mostrar todos';
    b.onclick = () => todasAsRotas(VISIVEIS.size < D.routes.length);
