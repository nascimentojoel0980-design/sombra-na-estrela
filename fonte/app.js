...
function ligaArvores(map, op) {
  ...
  const ORCAMENTO = op.orcamento == null ? 60 : op.orcamento;  // celulas novas por passagem
  ...
  let INST = null, nInst = 0, assinatura = '', semTerreno = false, incompleto = false;
  ...
  function semeia() {
    ...
    for (let ty = ty0; ty <= ty1; ty++) {
      for (let tx = tx0; tx <= tx1; tx++) {
        // distancia do centro da celula ao centro do mapa
    ...
    for (const c of partes) {
      const n = Math.min(c.length, (TECTO - k) * 11);
      if (n <= 0) break;
      buf.set(c.subarray(0, n), k * 11);
      k += n / 11;
    }
    INST = buf; nInst = k;
    if (op.aoContar) op.aoContar(k, k >= TECTO, incompleto);
  }
  ...
  return {
    ...
    resumo() {
      ...
      return { n: nInst, celulas: celulas.size, niveis: niv, incompleto,
