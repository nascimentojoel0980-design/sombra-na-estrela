...
- **"Nem tem pedras nem verde nenhum" (22/09/2026, v21)**: (1) a regra do ardido apagava a rocha dentro dos polígonos (302 mil células só na Torre); a rocha não arde, fica classe 7 com os blocos. (2) o limiar de cinza (0,35) era alto: calibrado nas classes de fora na mesma cena (chão nu 0,22; rocha 0,33–0,37; pastagem seca 0,24; rasteiro 0,39–0,47; matagal 0,56–0,69; floresta 0,58–0,87) → ≤ 0,22 cinza (13), 0,22–0,40 erva a nascer (3), > 0,40 mato rasteiro a rebentar (6, 0,8 m modelados); árvores nunca. Nas finas: 269 km² cinza, 353 erva, 340 a rebentar. (3) boletim: a máscara dos polígonos pós-fontes (`QARD`) calcula-se cedo; o teste da floresta nunca amostra lá dentro e fica NÃO VERIFICADO acima de 10 % (a foto é de antes do fogo; c4r1 chumbava com d=0,58 porque a "erva" de hoje era pinhal na foto).
- **Gerador de circulares** (`fonte/scripts-dados/rota_sombra.py`, 23/09/2026): grafo das vias OSM do armazém; sombra por troço = copa medida (ALTV ≥ 5 m) OU sombra do monte às horas pedidas (mapa HORI + posição do sol, a mesma fórmula do motor); custo = comprimento × factor da via × (1 + 2·(1 − sombra)); ida por Dijkstra e volta com as arestas da ida a 6×; escolhe pela sombra com 15–20 km e ≥ 70 % sem repetir. Por defeito evita povoações (células urbanas ou ruas fora de 1,2 km de casa custam 8×, máximo 1 km; `--cidade` desliga). Casa do Joel: -7.5244444 40.2409778 (Bairro do Cabeço, Tortosendo, 561 m; de `recomposicao-corporal/scripts/build_trilhos.py`). Escreve GPX+JSON; `scratchpad/desenha_rota.py` faz o mapa 2D com perfil e `foto_rota.js` a vista 3D. **Pendente**: pôr o gerador na app (o Joel quer, mas fica para depois).
